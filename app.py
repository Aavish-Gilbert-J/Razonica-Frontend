        ...
        if st.button("Sign In"):
            if username and password:
                with st.spinner("Authenticating..."):
                    ...
                    response = requests.post('https://1e9e-116-75-84-132.ngrok-free.app/login', json=data)
        ...
        if st.button("Create Account"):
            if username and password and confirm_password:
                ...
                with st.spinner("Creating your account..."):
                    data = {'username': username, 'password': password}
                    response = requests.post('https://1e9e-116-75-84-132.ngrok-free.app/register', json=data)
...
def display_files():
    ...
    response = requests.get('https://1e9e-116-75-84-132.ngrok-free.app/list_uploaded_files', headers=headers)
    if response.status_code == 200:
        ...
        if files_data:
            for file_info in files_data:
                ...
                if col3.button("Delete", key=f"delete_{upload_id}"):
                    ...
                    delete_response = requests.post('https://1e9e-116-75-84-132.ngrok-free.app/delete_upload', headers=headers, json=data)
...
def generate_response(prompt, options):
    ...
    try:
        run_response = requests.post('https://1e9e-116-75-84-132.ngrok-free.app/run_excel_agent', 
                                   headers=headers, 
                                   json=payload,
...
def main():
    if st.session_state['token']:
        ...
        <body>
            <form action="https://1e9e-116-75-84-132.ngrok-free.app/upload" class="dropzone" id="fileDropzone">
                    ...
                    init: function() {{
                        ...
                        this.on("addedfile", function(file) {{
                            ...
                            deleteIcon.addEventListener("click", function(e) {{
                                ...
                                fetch('https://1e9e-116-75-84-132.ngrok-free.app/delete', {{
                                    method: 'POST',
                                    headers: {{
                                        'Content-Type': 'application/json',
        ...
        """
            ...
            headers = {'Authorization': 'Bearer ' + st.session_state['token']}
            response = requests.get('https://1e9e-116-75-84-132.ngrok-free.app/get_user_files', headers=headers)
            if response.status_code == 200:
                files_data = response.json().get('files', [])
